...
if (audRate) {
  let amountEur = euros * rates.AUD;
  console.log("100 Euros is " + audRate + " AUD");
} else {
  console.log("AUD rate not found in rates object");
}
...
// TODO print Gary Oldman's cat's name by looping over the array until you find the object with name = "Gary Oldman"
  for (let i = 0; i < cats_and_owners.length; i++) {
    if (cats_and_owners[i].name === "Gary Oldman") {
      console.log("Gary Oldman's cat is called " + cats_and_owners[i].cat);
      break; // exit the loop once we find the owner
    }
  }
...
let literatureLaureate = nobel_prize_winners_2017.prizes[3].laureates[0];
...
let prizeCategories = nobel_prize_winners_2017.prizes.map(function(prize) {
...
console.log("Total Prize Categories: " + totalCategories);
...
console.log("Total Laureates: " + totalLaureates);
